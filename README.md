...
# Using StyleGAN2-ADA to improve classification accuracy on small datasets 
...
In this project we will show that for a small sized dataset of 2 classes (?dog and ?cat), with 500 samples for each, we will reach higher classification accuracy if we first use the StyleGAN2-ADA to create more artificial data and train the classifier on it, than to train the classifier only on the original data.
